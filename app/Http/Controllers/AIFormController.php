...
class AIFormController extends Controller
{
    ...
    4. Keep BLUEPRINT's structure, but only include code relevant to FIELD REFERENCE fields.";
        ...
        $payload[] = "CONTROLLER BLUEPRINT (Reference for validation patterns and parameter structure only):\n" . file_get_contents($controllerBlueprint) . "\n\nINSTRUCTIONS: Use this ONLY as a reference for validation rule patterns and parameter structure. Extract ONLY the validation rules and patterns for fields that exist in FIELD REFERENCE. Do NOT copy entire controller classes or methods. Do NOT include any scripts or JavaScript.";
        ...
        // Return a StreamedResponse with saving capability
        return new StreamedResponse(function () use ($model, $payload, $formName, $version, $currentSessionId, $previousForm, $isIncremental) {
            ...
            foreach ($stream as $response) {
                ...
                // Force the output buffer to send the chunk to the browser
                if (ob_get_level() > 0) {
                    ob_flush();
                }
                flush();
            ...
            // Save the generated code to database after streaming completes
            try {
